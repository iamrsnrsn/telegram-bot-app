...
async function loadPendingWithdrawals() {
    ...
    if (withdrawals.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:var(--text-secondary);">No pending withdrawals</p>';
        return;
    }
    
    withdrawals.forEach(withdrawal => {
        const item = document.createElement('div');
        item.className = 'withdrawal-admin-item';
        item.innerHTML = `
            <div class="withdrawal-admin-header">
                <div>
                    <div style="font-weight:600;">${withdrawal.amount.toFixed(2)}</div>
                    <div style="font-size:var(--font-size-sm);color:var(--text-secondary);">User: ${withdrawal.userId}</div>
                    <div style="font-size:var(--font-size-sm);color:var(--text-secondary);word-break:break-all;">Wallet: ${withdrawal.wallet}</div>
                    <div style="font-size:var(--font-size-xs);color:var(--text-secondary);margin-top:var(--space-1);">
                        ${new Date(withdrawal.requestedAt).toLocaleString()}
                    </div>
                </div>
                <div class="withdrawal-admin-actions">
                    <button class="btn btn-primary" onclick="approveWithdrawal('${withdrawal.reqId}')">Approve</button>
                    <button class="btn btn-secondary" onclick="rejectWithdrawal('${withdrawal.reqId}')">Reject</button>
                </div>
            </div>
        `;
        container.appendChild(item);
...
async function approveWithdrawal(reqId) {
    try {
        const response = await fetch('/api/withdraw/approve', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: state.trustedUserId, reqId })
        });
...
async function rejectWithdrawal(reqId) {
    try {
        const response = await fetch('/api/withdraw/reject', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: state.trustedUserId, reqId })
        });
